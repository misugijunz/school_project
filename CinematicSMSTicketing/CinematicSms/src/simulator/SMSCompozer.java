 ...
 * @author
 */
public class SMSCompozer extends Canvas implements CommandListener {
     ...
     */
    public SMSCompozer(Display display) {
    ...
    public String sendSMSSimulator(final String source, final int port) throws IOException {
        if (phoneNumber == null) {
            throw new IllegalArgumentException();
        }
        String address = "socket://" + source + ":" + port;
        //StreamConnection smsconn = null;
        /** Open the message connection. */
        //smsconn = (StreamConnection) Connector.open(address, Connector.READ_WRITE);
       //TextMessage txtmessage = (TextMessage) smsconn.newMessage(MessageConnection.TEXT_MESSAGE);
        //txtmessage.setAddress(address);
       // txtmessage.setPayloadText(message);
        //smsconn.send(txtmessage);

        SocketConnection conn = (SocketConnection) Connector.open(address);
        conn.setSocketOption(SocketConnection.DELAY, 0);
        InputStreamReader is = new InputStreamReader(conn.openDataInputStream());
        OutputStream os = conn.openDataOutputStream();
        System.out.println("Segera kirim data");
        os.write(getMessage().getBytes());
        System.out.println("Setelah kirim data");
        char c[] = new char[160];
        StringBuffer sb = new StringBuffer();
        /*while (! is.ready()) System.out.println("Data belum siap diterima");
        try {
            while (is.read(c,0,c.length) != -1) {
                  //System.out.println("Proses data");
                // memproses data yang diterima
                //c = is.readChar();
                System.out.println("baca data data");
                sb.append(c);
                System.out.println("baca buffer: " + c);
            }
        }
        catch(EOFException eofe) {}*/

        KeywordParser kp = new KeywordParser("011011");

        sb.append(kp.getResponse(getMessage()));

        String response = sb.toString();

        //InputStream is = smsconn.openInputStream();
        //OutputStream os = smsconn.openOutputStream();
